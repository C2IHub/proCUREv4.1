...
const AGENT_ID = process.env.RISK_PREDICTOR_AGENT_ID;
const AGENT_ALIAS_ID = process.env.RISK_PREDICTOR_AGENT_ALIAS_ID || 'TSTALIASID';
...
/**
 * Generic Lambda handler for Risk Predictor Agent
...
export const handler = async (event: LambdaEvent): Promise<LambdaResponse> => {
  ...
  if (!AGENT_ID) {
    console.error('RISK_PREDICTOR_AGENT_ID environment variable not set');
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ 
        error: 'Agent not configured',
        details: 'RISK_PREDICTOR_AGENT_ID environment variable missing'
      })
    };
  }
  ...
  try {
    ...
    const sessionId = requestBody.sessionId || `risk-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    // Build enhanced prompt with context for risk analysis
    let enhancedPrompt = requestBody.prompt;
    if (requestBody.context) {
      const contextInfo = Object.entries(requestBody.context)
        .map(([key, value]) => `${key}: ${value}`)
        .join('\n');
      enhancedPrompt = `Context:\n${contextInfo}\n\nRisk Analysis Request: ${requestBody.prompt}`;
    }
    ...
    console.log('Invoking Risk Predictor Bedrock Agent:', {
      agentId: AGENT_ID,
      agentAliasId: AGENT_ALIAS_ID,
      sessionId: sessionId,
    ...
    let fullResponse = '';
    let confidence = 0.88; // Default confidence for risk predictions
    ...
    // Add default risk analysis sources if none were found
    if (sources.length === 0) {
      sources.push(
        'Financial Market Data',
        'Credit Rating Agencies',
        'Industry Risk Benchmarks',
        'Regulatory Risk Databases',
    ...
    const agentResponse: AgentResponse = {
      ...
      sources: sources.slice(0, 5), // Limit to top 5 sources for risk analysis
      trace: process.env.NODE_ENV === 'development' ? traces : undefined
    };

    console.log('Risk Predictor response generated:', {
      sessionId: sessionId,
    ...
    console.error('Error invoking Risk Predictor Bedrock Agent:', error);
    ...
    if (error instanceof Error) {
      ...
      if (error.name === 'ValidationException') {
        ...
        statusCode = 404;
        errorMessage = 'Risk Predictor Agent not found or not accessible';
